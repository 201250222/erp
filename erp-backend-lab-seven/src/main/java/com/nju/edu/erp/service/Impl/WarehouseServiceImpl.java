...
public class WarehouseServiceImpl implements WarehouseService {
    ...
    private final WarehousePresentSheetDao wareHousePresentSheetDao;
    private final ProductService productService;


    @Autowired
    public WarehouseServiceImpl(ProductDao productDao, WarehouseDao warehouseDao, WarehouseInputSheetDao warehouseInputSheetDao, WarehouseOutputSheetDao warehouseOutputSheetDao, WarehousePresentSheetDao wareHousePresentSheetDao, ProductService productService) {
        this.productDao = productDao;
        this.warehouseDao = warehouseDao;
        this.warehouseInputSheetDao = warehouseInputSheetDao;
        this.warehouseOutputSheetDao = warehouseOutputSheetDao;
        this.wareHousePresentSheetDao = wareHousePresentSheetDao;
    ...
    public List<WarehouseIODetailPO> getWarehouseIODetailByTime(String beginDateStr,String endDateStr) {
        ...
        try{
            ...
            if(endTime.compareTo(beginTime)<0) return null;
    ...
    public void excel(HttpServletResponse response) {
        List<WarehouseCountingVO> vos = warehouseCounting();
        try {
            // 设置响应输出的头类型及下载文件的默认名称
            String fileName = new String("库存盘点.xls".getBytes(StandardCharsets.UTF_8), StandardCharsets.ISO_8859_1);
            response.addHeader("Content-Disposition", "attachment;filename=" + fileName);
            response.setContentType("application/vnd.ms-excel;charset=gb2312");
            //导出
            Workbook workbook = ExcelExportUtil.exportExcel(new ExportParams(), WarehouseCountingVO.class, vos);
    ...
    @Override
    public void warehousePresent(String presentId, Integer presentNumber, String operator, String saleSheetId) {
        ProductInfoVO present = productService.getOneProductByPid(presentId);
        WarehousePresentSheetPO latest = wareHousePresentSheetDao.getLatest();
        String id = IdGenerator.generateSheetId(latest==null? null: latest.getId(), "ZSD");
        WarehousePresentSheetPO warehousePresentSheetPO = WarehousePresentSheetPO.builder()
                .id(id)
                .presentId(presentId)
                .presentNum(presentNumber)
                .saleSheetId(saleSheetId)
                .operator(operator)
                .createTime(new Date())
                .value(present.getPurchasePrice().multiply(new BigDecimal(presentNumber)))
                .state(WarehousePresentSheetState.PENDING)
                .build();
        wareHousePresentSheetDao.save(warehousePresentSheetPO);
    }

    @Override
    public void approvalPresentSheet(UserVO user, String sheetId, WarehousePresentSheetState state) {
        WarehousePresentSheetPO warehousePresentSheetPO = wareHousePresentSheetDao.getById(sheetId);
        warehousePresentSheetPO.setOperator(user.getName());
        warehousePresentSheetPO.setState(state);
        wareHousePresentSheetDao.updateById(warehousePresentSheetPO);
        if(state==WarehousePresentSheetState.SUCCESS){
            String presentId = warehousePresentSheetPO.getPresentId();
            Integer presentNumber = warehousePresentSheetPO.getPresentNum();
            ProductInfoVO present = productService.getOneProductByPid(presentId);
            List<WarehouseOutputFormContentVO> list = new ArrayList<>();
            list.add(new WarehouseOutputFormContentVO(presentId,present.getPurchasePrice(),presentNumber,-1,"present"));
            WarehouseOutputFormVO form = new WarehouseOutputFormVO(list, warehousePresentSheetPO.getOperator(), warehousePresentSheetPO.getSaleSheetId());
            this.productOutOfWarehouse(form);
        }
    }

    @Override
    public List<WarehousePresentSheetVO> getWarehousePresentSheetsByState(WarehousePresentSheetState state) {
        List<WarehousePresentSheetPO> l;
        List<WarehousePresentSheetVO> ans = new ArrayList<>();
        if(state==null){
            l = wareHousePresentSheetDao.findAll();
        }
        else {
            l = wareHousePresentSheetDao.findAllByState(state);
        }
        for(WarehousePresentSheetPO warehousePresentSheetPO: l){
            WarehousePresentSheetVO warehousePresentSheetVO = new WarehousePresentSheetVO();
            BeanUtils.copyProperties(warehousePresentSheetPO,warehousePresentSheetVO);
