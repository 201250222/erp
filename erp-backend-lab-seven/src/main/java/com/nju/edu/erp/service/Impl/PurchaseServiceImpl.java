...
import com.nju.edu.erp.service.sheetService.PurchaseService;
...
public class PurchaseServiceImpl implements PurchaseService {
    ...
    @Override
    @Transactional
    public void makeSheet(UserVO userVO, SheetVO purchaseSheetVO) {
        ...
        for(PurchaseSheetContentVO content : ((PurchaseSheetVO)purchaseSheetVO).getPurchaseSheetContent()) {
    ...
    public List<SheetVO> getSheetByState(BaseEnum state) {
        ...
        if(state == null) {
            ...
            all = purchaseSheetDao.findAllByState((PurchaseSheetState) state);
        }
    ...
    public void approval(String purchaseSheetId, BaseEnum state) {
        if(state.equals(PurchaseSheetState.FAILURE)) {
            ...
            int effectLines = purchaseSheetDao.updateState(purchaseSheetId, (PurchaseSheetState) state);
            ...
            int effectLines = purchaseSheetDao.updateStateV2(purchaseSheetId, prevState, (PurchaseSheetState) state);
            if(effectLines == 0) throw new RuntimeException("状态更新失败");
            if(state.equals(PurchaseSheetState.SUCCESS)) {
                // TODO 审批完成, 修改一系列状态
                // 更新商品表的最新进价
