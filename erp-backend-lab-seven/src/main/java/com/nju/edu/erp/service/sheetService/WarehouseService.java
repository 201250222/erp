...
public interface WarehouseService {
    ...
    /**
     * 制定库存赠送单 存储在出库单中标识为库存赠送单
     */
    void warehousePresent(String presentId, Integer presentNumber, String operator, String saleSheetId);
    /**
     * 审批赠送单(仓库管理员进行确认)
     * @param sheetId 赠送但单id
     * @param state 赠送但单修改后的状态(state == "待审批"/"审批失败"/"审批完成")
     */
    void approvalPresentSheet(UserVO user, String sheetId, WarehousePresentSheetState state);

    /**
     * 通过状态获取赠送单(state == null 时获取全部赠送单)
     * @param state 赠送单状态
     * @return 赠送单
     */
    List<WarehousePresentSheetVO> getWarehousePresentSheetsByState(WarehousePresentSheetState state);
}
